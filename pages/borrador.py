...
def _tab_agregar():
    ...
    if st.button("📥 Agregar lista", use_container_width=False):
...
def _buscar_y_mostrar_candidatos(query: str, cantidad: int):
    ...
    for i, c in enumerate(candidatos[:8]):
        ...
        with col_add:
            if st.button("➕", key=f"add_{c['codigo']}_{i}", use_container_width=True,
...
def _tab_borrador():
    ...
    with col_del:
        if st.button("🗑️ Limpiar descartados", use_container_width=True):
            execute_query(
    ...
    with col_match:
        if st.button("🔍 Re-matching pendientes", use_container_width=True):
...
def _tab_exportar():
    ...
    with col_lote:
        ...
        if st.button("🛒 Crear lote de compra", use_container_width=True, type="primary"):
